...
if option[:d]
  ...
  if force_flag or site_name
    ...
    exec_command("git push heroku \`git subtree split --prefix #{dir_name} master\`:master --force",
                  "try build #{dir_name}",
                  site_name ? "show page: https://#{site_name}.herokuapp.com/" : "build success")
    exec_command("heroku pg:reset DATABASE", "reset database") if not migrate_flag
    exec_command("heroku run rails db:migrate", "migrate!") if not migrate_flag
    exec_command("heroku run rails db:seed", "init data") if not migrate_flag
    exec_command("heroku restart", "heroku restart") if not migrate_flag
